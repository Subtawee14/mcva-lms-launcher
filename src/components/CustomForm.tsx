...
const CustomForm: React.FC = () => {
    ...
    const [currentEnvironment, setCurrentEnvironment] = useState<keyof typeof mapUrl>("local");
    ...
    useEffect(() => {
        const fetchCourses = async () => {
            const response = await fetch(`${mapUrl[currentEnvironment].backend}/api/v1/courses`);
    ...
    const onSubmit = async () => {
        ...
        const token = await new jose.SignJWT(values).setProtectedHeader({ alg: "HS256", typ: "JWT" }).sign(secret);

        const hiddenForm = document.getElementById("hidden-form") as HTMLFormElement;

        let hiddenInput = document.getElementsByName("token")[0];
        hiddenForm.setAttribute("action", `${mapUrl[values.environment as keyof typeof mapUrl].frontend}/api/connect`);
        if (!hiddenInput) {
